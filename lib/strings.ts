const timerStatus = {
  running: "הטיימר פועל",
  paused: "הטיימר מושהה",
  finished: "הטיימר הסתיים",
} as const;

const foldProgressSuffix = "קיפולים בוצעו";

export const strings = {
  common: {
    cancel: "ביטול",
    confirm: "אישור",
    save: "שמור",
    delete: "מחק",
    close: "סגור",
    undo: "בטל",
    loading: "טוען…",
    saving: "שומר…",
    decrement: "פחות",
    increment: "עוד",
  },
  validation: {
    required: "שדה חובה",
    nameRequired: "השם לא יכול להיות ריק",
    positiveAmount: "הכמות חייבת להיות חיובית",
    emailInvalid: "כתובת אימייל לא תקינה",
  },
  welcome: {
    title: "נעים להכיר 👋",
    subtitle: "עוד רגע אופים. ספרו לנו רק מי אתם:",
    nameLabel: "שם פרטי",
    namePlaceholder: "איך קוראים לך?",
    emailLabel: "אימייל",
    privacyNote:
      "נשתמש בזה רק כדי להבין איך אופים עם כיכר ולעדכן אתכם על שיפורים. בלי ספאם.",
    cta: "מתחילים לאפות 🍞",
  },
  install: {
    title: "כיכר על מסך הבית",
    body: "כמו אפליקציה אמיתית — והבייקים והמתכונים שלכם נשמרים לאורך זמן.",
    installCta: "התקנה",
    iosCta: "איך מתקינים?",
    fbTitle: "אתם בדפדפן של פייסבוק",
    fbBody: "כדי להתקין את כיכר פתחו בדפדפן הרגיל: תפריט ⋯ למעלה ← ״פתיחה בדפדפן חיצוני״.",
    dismissLabel: "סגירת הצעת ההתקנה",
    guideTitle: "התקנה על אייפון",
    guideStep1: "לחצו על כפתור השיתוף בסרגל הדפדפן",
    guideStep2: "גללו ובחרו ״הוסף למסך הבית״",
    guideStep3: "אישור — וכיכר מחכה לכם במסך הבית",
  },
  home: {
    wordmark: "כיכר",
    subtitle: "מה אופים היום?",
    startBaking: "התחל אפייה",
    startBakingAlt: "אפייה חדשה",
    myRecipes: "המתכונים שלי",
    starterTracker: "מעקב סטארטר",
  },
  recipes: {
    pageTitle: "המתכונים שלי",
    newRecipe: "+ מתכון חדש",
    backToHome: "חזרה",
    emptyTitle: "עדיין אין מתכונים",
    emptyDescription: "הוסף את המתכון הראשון שלך כדי להתחיל",
  },
  bakeStub: {
    title: "מצב אפייה — בקרוב",
    description: "הזרימה הזאת תיכנס בקרוב. בינתיים, תכין את המתכונים שלך.",
  },
  bake: {
    chooserTitle: "בייק חדש",
    chooserRecipeHeading: "איזה סוג לחם?",
    myBadge: "שלי",
    // Banner (compact, on top of HomeScreen when there's an active bake)
    resumeBannerLabel: "ממשיכים",
    resumeBannerStage: (stageNumber: number, totalStages: number) =>
      `שלב ${stageNumber} מתוך ${totalStages}`,
    resumeBannerContinue: "המשך",
    resumeBannerStop: "סיים בייק",
    // Dialog: stopping the current bake (from the banner)
    stopTitle: "להפסיק את הבייק?",
    stopDescription: (recipeName: string) =>
      `ההתקדמות של ״${recipeName}״ תאבד.`,
    stopConfirm: "כן, להפסיק",
    stopCancel: "לא, להמשיך",
    // Dialog: starting a new bake while one is active (from the chooser)
    replaceTitle: "להחליף בייק?",
    replaceDescription: (recipeName: string) =>
      `הבייק הנוכחי של ״${recipeName}״ יסתיים.`,
    replaceConfirm: "כן, להחליף",
    replaceCancel: "ביטול",
    stagePlaceholderTitle: (stageNumber: number) => `שלב ${stageNumber} — בקרוב`,
    stagePlaceholderBackToHome: "חזרה למסך הבית",
    doneTitle: "הלחם מוכן!",
    doneBlurb: "תהנו מהפרי של 24–48 שעות עבודה. אחרי שעת הקירור על הרשת — הקרום עכשיו בשיאו.",
    doneButton: "סיימתי",
    // Stage screen
    stageCounter: (current: number, total: number) => `${current}/${total}`,
    stageNext: (nextName: string) => `הבא — ${nextName}`,
    stageNextShort: "הבא",
    stagePrev: "חזרה",
    stageMenuLabel: "תפריט",
    stageFinishFold: "סיימתי קיפול",
    stageDone: "סיימתי",
    timerStatus,
    foldProgressSuffix,
    foldProgress: (current: number, total: number) =>
      `${current} / ${total} ${foldProgressSuffix}`,
    // Stage-specific timer copy. Only stage 2 has approved wording today;
    // T6 adds the remaining stages rather than reusing autolyse hints.
    autolyseTimer: {
      idleHint: "אחרי שכל הקמח רטוב והקערה מכוסה, אפשר להפעיל טיימר למנוחה.",
      setupHint: "כמה זמן לתת לבצק לנוח? בדרך כלל מומלץ לבחור 30–60 דקות.",
    },
    // The shared timer shell — one shape for every stage that carries a wait.
    bakeTimer: {
      heading: "טיימר",
      setupTitle: "בחירת זמן",
      editTitle: "עריכת זמן שנותר",
      countdownTitle: "טיימר אוטוליזה",
      editHint: "בחרו כמה זמן יישאר בטיימר מרגע השמירה.",
      hoursLabel: "שעות",
      minutesLabel: "דקות",
      hoursOption: (hours: number) => `${hours} ${hours === 1 ? "שעה" : "שעות"}`,
      durationOption: (minutes: number) => `${minutes} דקות`,
      start: "הפעל טיימר",
      saveTime: "שמור זמן",
      timeRemaining: "זמן שנותר",
      openExpanded: "פתח טיימר מורחב",
      edit: "עריכת זמן",
      running: timerStatus.running,
      paused: timerStatus.paused,
      finished: timerStatus.finished,
      pause: "השהה",
      resume: "המשך",
      reset: "התחל מחדש",
    },
    // Dough temp shadow estimate (feature 22)
    doughTemp: {
      prompt: "יש מד-חום? מדדו את טמפ׳ הבצק",
      measured: "מדדתי",
      fieldLabel: "טמפ׳ הבצק",
      save: "שמירה",
      cancel: "ביטול",
      edit: "עריכה",
      remove: "הסרת המדידה",
      rangeError: "טווח סביר: 18–35°C",
      shadowPrefix: (temp: string) => `לפי טמפ׳ הבצק (${temp}°): `,
      hintEarly: "התחילו לבדוק מוקדם מהרגיל.",
      hintLater: "יש לכם יותר זמן מהרגיל.",
      hintSimilar: "קרוב לתחזית הרגילה.",
      signsRule: "הסימנים על הבצק עדיין קובעים.",
    },
    // Bulk readiness (feature 31)
    bulkDecisionRule:
      "מתלבטים? סיימו מוקדם. תת-תסיסה קלה מתקנת את עצמה בהתפחה הקרה — תסיסת-יתר לא.",
    bulkCompareTrigger: "לא בטוחים? השוו: חסר · מוכן · יתר",
    // Vertical media: a card in the flow, the player inside a sheet (feature 31, T2)
    stageVideo: {
      watchOnYouTube: "פתחו ב-YouTube",
      offline: "אין חיבור לאינטרנט כרגע. אפשר לפתוח את הסרטון ביוטיוב.",
      playerTitle: "סרטון הדגמה",
    },
    // Rescue paths (feature 20)
    rescueTrigger: "משהו לא מסתדר?",
    rescueSheetTitle: "אבחון מהיר",
    rescueSigns: "הסימנים",
    rescueSteps: "מה עושים",
    stageKnowledge: {
      calibration: {
        before: "מיד אחרי הערבוב – הבצק עדיין גס ולא אחיד.",
        after: "אחרי המנוחה – הבצק מחובר יותר ונמתח בקלות רבה יותר.",
        caveat:
          "המראה תלוי בקמח ובהידרציה; השוו את הבצק לעצמו בתחילת המנוחה, לא למראה קבוע אחד.",
      },
      guides: {
        bulk: {
          title: "להבין את התסיסה",
          triggerLabel: "הסבר על התסיסה הראשונית",
          intro:
            "התסיסה הראשונית עושה שתי פעולות במקביל: מייצרת גז שמנפח את הבצק, ובונה את המבנה שיחזיק אותו. המטרה היא ששני התהליכים יסתיימו יחד. בצק שתפח בלי מבנה יקרוס, ומבנה חזק ללא תפיחה ייתן לחם צפוף.",
          mechanism: {
            heading: "למה החום קובע?",
            body:
              "קצב התסיסה נקבע מטמפרטורת הבצק, ולכן אותו מתכון בדיוק יסתיים בזמנים שונים לחלוטין בקיץ ובחורף. השעון משמש רק כהערכה כללית. מה שקובע את סיום השלב הם הסימנים שאתם מזהים בקערה, לכן סמכו על מראה הבצק ותחושתו ואל תמתינו לזמן מסוים.",
          },
          folds: {
            heading: "תפקיד הקיפולים",
            body:
              "הקיפולים נועדו לבנות חוזק בבצק, והם אינם מאיצים את התסיסה עצמה. ככל שהבצק הופך חזק יותר, כך המרווחים בין הקיפולים הולכים וגדלים. מפסיקים לקפל ברגע שהבצק מצליח להחזיק את צורתו בין קיפול לקיפול. ההמתנה השקטה אחרי הקיפול האחרון היא חלק מהותי מהעבודה, ולא זמן מת — רוב פעילות התסיסה מתרחשת דווקא בזמן שהבצק פשוט נח.",
          },
          recipeContext: {
            heading: "מה זה אומר על הבצק שלכם?",
            guidance: {
              spelt:
                "כוסמין הוא קמח רך שנמתח מהר, ורשת הגלוטן שלו שבירה יותר. לכן, בצק מבוסס כוסמין נוטה להגיע לסוף התסיסה מוקדם מהצפוי. חפשו את הסימנים מוקדם ואל תחכו לשעון.",
              wholeWheat:
                "הסובין מאיץ את התסיסה אבל גם מכביד על הבצק. בגלל המשקל הזה, התפיחה יכולה להיראות פחות דרמטית לעין, גם כשהבצק פעיל לחלוטין ומוכן.",
              rye:
                "קמח שיפון תוסס מהר מאוד וכמעט אינו בונה רשת אלסטית. לכן, השתחררות מהדפנות וריטוט ייראו אחרת לגמרי, והסתמכות על תפיחה כסימן תהיה פחות אמינה.",
              highHydration:
                "כמות מים גבוהה הופכת את הבצק לרופס ודביק יותר. הוא מתפשט לרוחב במקום לעלות לגובה, ותחושת הריטוט תופיע אצלו מוקדם יותר מאשר בבצק נוקשה.",
              lowHydration:
                "בצק עם מעט מים הוא נוקשה יותר. תהליך התפיחה ייראה איטי יותר, העלייה בנפח תהיה פחות ברורה לעין, והריטוט כשתנערו את הקערה יהיה עדין מאוד.",
              generic:
                "בבצק מחיטה לבנה סימני התסיסה מתנהגים לפי הספר. הדרך הטובה ביותר היא להשוות את מצב הבצק לעצמו בתחילת השלב, ולא לחפש מראה אבסולוטי אחד.",
              warmKitchen:
                "בטמפרטורה של 26° ומעלה, הכול רץ מהר יותר וקל לפספס את החלון המדויק. חפשו את הסימנים מוקדם מהרגיל כדי לא להגיע למצב של תסיסת-יתר.",
            },
          },
        },
        autolyse: {
          title: "להבין את הבצק",
          triggerLabel: "הסבר על אוטוליזה",
          intro:
            "המטרה בשלב הזה היא לא לפתח את הבצק עד הסוף, אלא לתת לקמח לשתות מים ולהתארגן כדי להקל על הערבוב בהמשך.",
          mechanism: {
            heading: "מה קורה בקערה?",
            body:
              "בזמן המנוחה המים נספגים ונוצרים קשרים בין חלבוני החיטה, ובו זמנית אנזימים מרככים את הבצק. ההתרככות הזו עוזרת למתיחה, אבל יש לה מחיר: מנוחה ארוכה מדי, במיוחד ביום חם, עלולה להחליש את הבצק. יותר זמן לא תמיד שווה לתוצאה טובה יותר.",
          },
          graph: {
            title: "הרווח מול הסיכון בזמן האוטוליזה",
            badge: "להמחשה בלבד",
            startLabel: "תחילת המנוחה",
            endLabel: "מנוחה ממושכת",
            hydrationLabel: "ספיגת מים והתארגנות",
            weakeningLabel: "סיכון להיחלשות",
            description:
              "גרף הממחיש שעם הזמן, הרווח מספיגת המים הולך ומתמתן, בעוד שהסיכון להיחלשות הבצק ממשיך לעלות.",
          },
          recipeContext: {
            heading: "מה זה אומר על הבצק שלכם?",
            guidance: {
              spelt:
                "בצק שמבוסס על כוסמין נוטה להיות רך ולהימתח יותר מבצק מחיטת לחם. אל תחכו שהמנוחה תחזק אותו; חפשו חיבור ראשוני.",
              wholeWheat:
                "הסובין ממשיך לשתות מים, ולכן המרקם יכול להישאר מחוספס. חפשו שהקמח רטוב והבצק מחובר יותר; הוא לא צריך להיות חלק.",
              rye:
                "שיפון לא בונה רשת אלסטית כמו חיטה. המטרה כאן היא לספוג מים. אל תחכו שהבצק יימתח או יתחזק, ואל תאריכו את הזמן לשם כך.",
              generic:
                "ההתנהגות משתנה בין קמחים, אבל המטרה זהה. השוו את מצב הבצק לעצמו מיד לאחר הערבוב.",
              highHydration:
                "בהידרציה גבוהה תיתכן יותר דביקות, וזה טבעי. חפשו שהבצק מתחבר, לא יובש או מרקם חלק.",
              lowHydration:
                "בבצק נוקשה, שינוי המרקם יכול להיות עדין. רק ודאו שאין כיסי קמח יבשים ואל תוסיפו מים כעת.",
              warmKitchen:
                "במטבח חם הבצק עשוי להתרכך מהר יותר. אל תוסיפו זמן רק כדי להגיע למרקם חלק.",
            },
          },
          practicalCheck:
            "הרטיבו מעט את הידיים והרימו קצה קטן מהבצק כדי להרגיש אם הוא נמתח מעט יותר בקלות מאשר בתחילת המנוחה.",
          decisionRule:
            "אל תאריכו את זמן המנוחה רק כדי להעלים חספוס או להשיג בצק חלק.",
        },
      },
    },
    flourTypeLabels: {
      white: "קמח לבן",
      wholeWheat: "קמח מלא",
      rye: "קמח שיפון",
      speltWhite: "קמח כוסמין לבן",
      speltWhole: "קמח כוסמין מלא",
      other: "קמח אחר",
    },
    bakingMethod: {
      selectorTitle: "באיזה כלי תאפה?",
      methods: {
        "closed-vessel": {
          title: "סיר/כלי סגור",
          description:
            "סיר ברזל יצוק, סיר נירוסטה עם מכסה, או תבנית עם קערה הפוכה. הכלי אוטם את האדים.",
        },
        "open-with-steam": {
          title: "אפייה פתוחה + תבנית אדים",
          description:
            "תבנית/אבן/פלדה/סיר נירוסטה ללא מכסה, עם תבנית מים נפרדת למטה לאדים.",
        },
        other: {
          title: "אחר / לא בטוח",
          description: "ההוראות הגנריות יעזרו לכם להבין מה לעשות לפי הציוד שלכם.",
        },
      },
      genericSafetyWarning:
        "ודאו שכל הציוד שאתם משתמשים בו עמיד לחום של 250°C. מתכת (נירוסטה) או קרמיקה איכותית בטוחות. זכוכית רגילה תיסדק. פלסטיק יימס.",
    },
  },
  form: {
    nameLabel: "שם המתכון",
    namePlaceholder: "לחם של שישי",
    flourBreakdownTitle: "פירוט קמחים",
    flourWhite: "לבן",
    flourWholeWheat: "מלא",
    flourRye: "שיפון",
    flourSpeltWhite: "כוסמין לבן",
    flourSpeltWhole: "כוסמין מלא",
    flourSumOk: "סה״כ: ",
    flourSumShort: "חסר",
    flourSumOver: "עודף",
    flourWeight: "משקל קמח",
    flourWeightHintMedium: "500g · ככר בינונית (~1kg בצק)",
    flourWeightHintLarge: "750g · ככר גדולה (~1.5kg בצק)",
    hydration: "הידרציה",
    hydrationFlourNote:
      "קמח לבן רגיל מהסופר (~10.5% חלבון) סופג פחות מים — איתו הישארו סביב 70–72%. קמח לחם (12%+ חלבון) מחזיק גם 75% ומעלה.",
    salt: "מלח",
    levain: "שאור",
    kitchenTemp: "טמפ׳ מטבח",
    saveButton: "שמור",
    cancelButton: "ביטול",
    inclusionsTitle: "תוספות (אופציונלי)",
    inclusionsAdd: "+ הוסף תוספת",
    inclusionName: "שם",
    inclusionNamePlaceholder: "זיתים שחורים",
    inclusionAmount: "כמות",
    unitGrams: "גרם",
  },
  bakeConfirm: {
    title: "מוכן לאפות?",
    quantities: "כמויות לבייק",
    flour: "קמח",
    water: "מים",
    levain: "שאור",
    salt: "מלח",
    tempLabel: "טמפ׳ מטבח",
    timingHeading: "זמנים משוערים",
    timingSuffix: "(תלוי טמפ׳)",
    startButton: "התחל בייק",
    recommendedNote: "כמויות מומלצות",
    editButton: "ערוך מתכון",
    savedReturnToast: "המתכון נשמר — בחר אותו כאן להתחלת הבייק",
  },
  starterGate: {
    // Gate screen — the yes/no question
    gateTitle: "לפני שמתחילים",
    gateQuestion: "הסטארטר שלך בשיא?",
    gateEducationBlurb:
      "סטארטר בשיא הוכפל בנפח, מלא בועות, ריחו חמצמץ-מתוק, ועובר מבחן ציפה (float test).",
    gateCta_yes: "כן, הסטארטר בשיא",
    gateCta_no: "לא, צריך לתכנן",
    // Schedule screen — planning path
    scheduleTitle: "מתכננים את הבייק",
    scheduleReadyLabel: "מתי הלחם צריך להיות מוכן?",
    scheduleContextLine: (hours: number) =>
      `מהאכלת הסטארטר ועד לחם מוכן — כ-${hours} שעות`,
    schedulePillEarliest: "הכי מוקדם",
    scheduleTempLabel: "טמפ׳ מטבח (°C)",
    scheduleTooSoon: (minDate: string) =>
      `המינימום לבייק זה הוא ${minDate}. הסטארטר + הבייק לוקחים לפחות 38 שעות.`,
    scheduleDismiss: "הבנתי, אחזור מאוחר יותר",
    // Feeding window card
    feedCardTitle: "תוכנית האכלה",
    feedCardFeedLabel: "האכלת הסטארטר",
    feedCardPeakLabel: "הסטארטר יגיע לשיא",
    feedCardDurationBridge: (hours: number) => `כ-${hours} שעות`,
    feedCardTimeDisclaimer: "הזמן הוא הערכה — הסטארטר עצמו יגיד לך מתי הוא מוכן",
    feedCardReadinessTitle: "איך יודעים שהסטארטר מוכן?",
    feedCardReadinessSigns:
      "הוכפל בנפח · מלא בועות · ריח חמצמץ-מתוק · שקוע מעט בפסגה · עובר מבחן ציפה (float test)",
    feedCardCalcNote: (temp: number) => `ההערכה מחושבת לפי ${temp}°C`,
    // Peak explainer — planner toggle trigger + sheet (feature 13)
    peakInfoTrigger: "מה זה בשיא?",
    peakInfoAriaLabel: "פתח הסבר — מה זה סטארטר בשיא",
    peakSheetTitle: "סטארטר בשיא — 4 הסימנים",
    peakSign1: "הוכפל בנפח מאז ההאכלה האחרונה",
    peakSign2: "מלא בועות אוויר",
    peakSign3: "ריח חמצמץ-מתוק (לא חומצי חד)",
    peakSign4: "עובר מבחן ציפה — כפית בכוס מים, הסטארטר צף",
    peakImageAlt: "שלושה מצבי סטארטר: שטוח לפני השיא, מבעבע בשיא, קרוס אחרי השיא",
  },
  bakeScheduler: {
    headerTitle: "תכנון הבייק",
    planningTitle: "תכננו את הבייק",
    planningSubtitle: "הגדירו את התנאים והזמנים שמתאימים לכם",
    backToChooser: "חזרה",
    starterLabel: "הסטארטר כבר בשיא?",
    starterYes: "כן",
    starterNo: "לא",
    readyQuestion: "מתי שהלחם ייצא מהתנור?",
    earliest: "הכי מוקדם",
    tempQuestion: "מהי טמפרטורת החדר?",
    tempHint: "חורף 20–24° · קיץ 24–27°",
    tempImportantHint: "הטמפרטורה קובעת את כל הזמנים — כדאי למדוד ולא לנחש",
    scheduleSectionTitle: "תכננו את זמני הבייק",
    scheduleSectionSubtitle: "בחרו תבנית מומלצת או כווננו ידנית",
    presetRowLabel: "התחל מתבנית",
    presetReadyLabel: (day: string, time: string) => `${day} · ${time}`,
    advancedDisclosureOpen: "לכוונן בעצמי",
    advancedDisclosureClose: "סגור כוונון",
    presets: {
      fast:        { name: "מהיר",          hint: "מוכן הכי מהר · טעם עדין" },
      classic:     { name: "קלאסי",         hint: "לילה במקרר · מוכן בבוקר" },
      classicLate: { name: "קלאסי מאוחר",  hint: "לילה ארוך · מוכן לקראת ערב" },
      long:        { name: "ארוך",           hint: "התפחה ממושכת · חמצמץ ועשיר" },
      manual:      { name: "לכוונן בעצמי",  hint: "שלטו בזמנים ויחס האכלת הסטארטר" },
    },
    ratioControl: {
      label: "יחס בניית שאור (levain)",
      hint: (feedHour: string) => `שיא ב-${feedHour}`,
    },
    directionEnd: "מתי לסיים",
    directionStart: "מתי להתחיל",
    readyQuestionStart: "מתי מתחילים?",
    exactTimeLabel: "בחירת שעה מדויקת",
    pastHint: "אפשר לבחור גם שעה שכבר עברה — אם התחלתם קודם",
    contextLineStart: (hours: number) => `הלחם יהיה מוכן בעוד כ-${hours} שעות`,
    readyResultLabel: (label: string) => `הלחם מוכן ב: ${label}`,
    contextLine: (hours: number) => `הבייק ייקח כ-${hours} שעות`,
    // Inline retard editing on the timeline
    retardSliderLabel: "אורך ההתפחה במקרר",
    retardOverflowNote: (label: string) =>
      `כדי להספיק התפחה כזו, הלחם ייצא ב-${label}`,
    // Timeline
    timelineTitle: "ציר הזמן של הבייק",
    timelineSubtitle: "מתי להתחיל כל שלב",
    timelineEstimateNote: "⏱ זמנים משוערים — עשויים להשתנות לפי הטמפרטורה והסטארטר",
    coolingTip: "מומלץ לצנן כ-שעה לפני חיתוך — חיתוך חם הורס את הפירור",
    // Per-step copy (key matches BakeStepKey). desc may be omitted when computed from duration.
    timelineSteps: {
      build: { label: "בניית שאור (levain)", desc: "ריענון הסטארטר עד לשיא" },
      mix: { label: "ערבוב", desc: "אוטוליזה 45 דק׳ + לישה 15 דק׳" },
      bulk: { label: "תסיסה ראשונית", desc: "כולל קיפולים" },
      shape: { label: "עיצוב הבצק", desc: "עיצוב + מנוחת ביניים · כ-35 דק׳" },
      retard: { label: "התפחה במקרר", desc: "בלילה" },
      preheat: { label: "חימום התנור והכלי", desc: "כ-45 דקות" },
      bake: { label: "הכנסת הבצק לתנור", desc: "מכוסה 20 דק׳ + חשוף 22 דק׳" },
      ready: { label: "הלחם מוכן" },
    },
    tooSoon: (minDate: string) => `המוקדם ביותר: ${minDate}`,
    startButton: "התחל בייק",
    compactSummary: {
      feedLabel: "בניית שאור (levain)",
      activeWindowLabel: "עבודה אקטיבית",
      readyLabel: "הלחם מוכן",
      showTimeline: "הצג את כל השלבים",
    },
  },
  validationCopy: {
    nameRequired: "שדה חובה",
    flourSumWrong: (current: number) =>
      `סה״כ הקמחים חייב להיות 100% (כרגע: ${current.toFixed(1)}%)`,
    hydrationRange: "הידרציה צריכה להיות בין 50% ל-100%",
    saltRange: "מלח צריך להיות בין 0% ל-5%",
    levainRange: "שאור צריך להיות בין 0% ל-40%",
    flourWeightRange: "משקל קמח חייב להיות בין 100g ל-1500g",
    tempRange: "טמפ׳ צריכה להיות בין 10°C ל-40°C",
    inclusionNameRequired: "שם התוספת לא יכול להיות ריק",
    inclusionAmountPositive: "כמות התוספת חייבת להיות חיובית",
  },
  starterTracker: {
    grams: {
      starterLabel: "סטארטר",
      flourLabel: "קמח",
      waterLabel: "מים",
      unit: "גרם",
    },
    validation: {
      ratioRequired: "בחרו יחס האכלה",
      dateRequired: "תאריך ההאכלה הוא שדה חובה",
      gramsNegative: "הכמות לא יכולה להיות שלילית",
    },
    form: {
      addTitle: "האכלה חדשה",
      editTitle: "עריכת האכלה",
      backToList: "חזרה",
      gramsSectionTitle: "פירוט במשקל (לא חובה)",
      dateLabel: "תאריך",
      timeLabel: "שעה",
      saveButton: "שמור",
      cancelButton: "ביטול",
      deleteButton: "מחק",
      savedToast: "ההאכלה נשמרה",
      saveErrorToast: "לא הצלחנו לשמור — נסה שוב",
      deletedToast: "ההאכלה נמחקה",
      deleteErrorToast: "לא הצלחנו למחוק — נסה שוב",
    },
    deleteDialog: {
      title: "למחוק את ההאכלה הזו?",
      description: "הפעולה לא ניתנת לביטול",
      confirm: "מחק",
      cancel: "ביטול",
    },
    list: {
      pageTitle: "מעקב סטארטר",
      backToHome: "חזרה",
      newFeeding: "+ האכלה חדשה",
      loadErrorMessage: "משהו השתבש בטעינת ההיסטוריה",
      retry: "נסה שוב",
    },
    empty: {
      emptyTitle: "עדיין לא תיעדת האכלות",
      emptyDescription: "הוסף את ההאכלה הראשונה שלך כדי להתחיל לעקוב",
      newFeeding: "+ האכלה ראשונה",
    },
  },
  feedback: {
    fabLabel: "שליחת פידבק",
    title: "שליחת פידבק",
    typeLabel: "סוג",
    typeBug: "באג",
    typeFeature: "הצעה לפיצ׳ר",
    typeQuestion: "שאלה",
    typeOther: "אחר",
    nameLabel: "שם (אופציונלי)",
    namePlaceholder: "השם שלך",
    descriptionLabel: "תיאור",
    descriptionPlaceholder: "תאר את הבעיה או ההצעה…",
    addImage: "הוסף תמונה",
    removeImage: "הסר תמונה",
    submit: "שלח פידבק",
    submitting: "שולח…",
    successToast: "הפידבק נשלח, תודה!",
    errorMessage: "שליחה נכשלה — נסה שוב",
  },
} as const;
