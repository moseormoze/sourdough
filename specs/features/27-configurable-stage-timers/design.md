...
- **`TimerProgress`** — פס התקדמות אמיתי, מחליף את `TimerSignal`. גובה 3px,
  מסילה `bg-paper/20`, מילוי לפי `secondsLeft / durationSeconds`. **בלי spring**
  (‏ui-playbook §4: לא לקפיץ progress bars) — הרוחב מתעדכן בשנייה.
...
## Open Question — דרושה הכרעה אחת (המשתמש)
