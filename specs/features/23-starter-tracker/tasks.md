...
- `lib/storage/feedings.ts` — חדש: `listFeedings(email)`, `createFeeding(input)`, `updateFeeding(id, input)`, `deleteFeeding(id, email)` — עוטף `lib/supabase/client.ts`. **עדכון (בוצע ב-T2)**: `deleteFeeding` ו-`updateFeeding` מקבלים `email` ומסננים לפיו גם ב-query עצמו (לא רק `id`) — כדי לעמוד ב-Done-when "כל query מסונן לפי אימייל", שלא היה אפשרי עם `deleteFeeding(id)` בלבד
...
### T7 — Auto-calc: מילוי קמח/מים מהיחס (נוסף אחרי שחרור, פידבק משתמש)
**Goal:** כשנבחר יחס ומוזן משקל סטארטר — קמח ומים מתמלאים אוטומטית (סטארטר × N). מילוי ידני של המשתמש תמיד גובר — האוטומציה לא דורסת ערך שהמשתמש הקליד בעצמו.

**Files likely touched:**
- `components/starter/feeding-form-screen.tsx` — לוגיקת auto-fill
- `components/starter/feeding-form-screen.test.tsx` — טסטים

**התנהגות:**
- הזנת משקל סטארטר כשיחס נבחר → `flourGrams = waterGrams = starterGrams × N`
- החלפת יחס כשמשקל סטארטר קיים → חישוב מחדש
- שדה שהמשתמש ערך ידנית (ערכו שונה מהערך האחרון שחושב אוטומטית) — לא נדרס
- ניקוי משקל הסטארטר → שדות שחושבו אוטומטית מתרוקנים (ידניים נשארים)
- במצב עריכה של רשומה קיימת: אין auto-fill על הטעינה הראשונית — רק על אינטראקציה חדשה

**Test strategy:** כל חמשת המקרים למעלה + רגרסיה על ולידציה/שמירה קיימות

**Depends on:** T4 (merged)

**Done when:**
- [ ] Tests written and passing
- [ ] עריכה ידנית לעולם לא נדרסת על ידי החישוב
- [ ] אין רגרסיה בטסטי הטופס הקיימים
