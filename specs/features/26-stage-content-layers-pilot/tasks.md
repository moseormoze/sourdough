...
**סטטוס:** T1 ו־T2 הושלמו ב־PRs #68 ו־#69. רק T3 נשאר למימוש. שינויי טיימר
עתידיים אינם חלק מ־T3 ומנוהלים בנפרד ב־Issue #72.
...
**Goal:** לתעד את המימוש שמוזג ב־PR #69: פסקת מטרה קצרה, ארבע פעולות, שלושה
סימנים ומשפט מעבר; להסיר את הפניית תמונת ה־AI; ולשלב את טיימר האוטוליזה שנבנה
באותו PR. T3 משמר את התוצאה הזו ואינו פותח אותה מחדש.
...
- `components/bake/autolyse-timer.tsx` והבדיקות שלו
- `components/bake/duration-wheel.tsx` והבדיקות שלו
- `components/bake/countdown-ring.tsx` והבדיקות שלו
...
שמוכיחות:
- שלב 2 מכיל בדיוק 0 takeaways, ארבע פעולות, שלושה סימני המשך ומשפט מעבר לא־ריק.
...
- שלב 2 מציג את `AutolyseTimer` שמוזג ב־PR #69; שינוי או החלפה שלו אינם חלק מ־T3.
- שלב 1 שומר את התוכן והמבנה הקיימים; שלב 3 ממשיך להשתמש ביתרת המים שנשקלה
  ונשמרה בשלב 2.
...
- [x] שקילת יתרת המים נשארת בשלב 2 והכמות נכנסת בשלב 3.
- [x] הקובץ `public/stages/2-autolyse.png` אינו חייב להימחק; הוא פשוט אינו מיוחס
  או מרונדר בפיילוט.
- [x] `AutolyseTimer` מחובר לפעולות הטיימר של הבייק כפי שמוזג ב־PR #69.
