-- Feature 23 (starter-tracker): feedings table.
-- Supabase is introduced early and narrowly for this table only — see
-- context/tech-stack.md and context/launch-plan.md for the scoped decision.
...
-- scoped to auth.uid()).
create policy "feedings_permissive_select" on feedings
  for select to anon using (true);

create policy "feedings_permissive_insert" on feedings
  for insert to anon with check (true);

create policy "feedings_permissive_update" on feedings
  for update to anon using (true) with check (true);

create policy "feedings_permissive_delete" on feedings
